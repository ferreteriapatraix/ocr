...
import { IFile } from './poto/File';
import * as handlebarsDropdownTemplate from './templates/dropdown.hbs';
...
export class View {
    ...
    private static readonly templateOCRSelectedFileAction: string = `
    ...
    private notificationRow: JQueryStatic<HTMLElement> | undefined = undefined;
     ...
     */
    public renderDropdown(languages: string[]): string {
        this.destroyDropdown();
        return handlebarsDropdownTemplate({ languages, buttonText: t('ocr', 'Process'), replaceText: t('ocr', 'Replace') });
    }
     ...
     */
    public renderFileAction(fileName: string, languages: string[]): void {
        const html = this.renderDropdown(languages);
        if (fileName !== undefined) {
            const trs: HTMLTableRowElement[] = [].slice.call(this.document.querySelectorAll('tr'));
    ...
    public renderSelectedFilesActionButton(): void {
        this.prependHtmlToElement(View.templateOCRSelectedFileAction, this.document.getElementById('selectedActionsList'));
    }
     ...
     */
    public getSelectTwoValues(): string[] {
    ...
    public getReplaceValue(): boolean {
        return (this.document.getElementById('ocrReplace') as HTMLInputElement).checked;
    }
